...
void updatePlayer() {
    ...
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<7))) && player.speed < 45) {
...
void drawPlayer() {
    ...
    shadowOAM[0].attr1 = ((player.col) & 0x1FF) | (2<<14);
    shadowOAM[0].attr2 = ((player.spriteY)*32+(player.spriteX));
}

void drawSelector(SELECTOR* c, int i) {
    shadowOAM[1+i].attr0 = ((c->row) & 0xFF) | (0<<13) | (0<<14);
    shadowOAM[1+i].attr1 = ((c->col) & 0x1FF) | (1<<14);
    shadowOAM[1+i].attr2 = ((c->spriteY)*32+(c->spriteX));
}
